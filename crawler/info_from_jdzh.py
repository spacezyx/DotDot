...
def getData(html):
    ...
    for item in soup.find_all():  # 查找符合要求的字符串
        ...
        for i in range(length):
            ...
            if(st == None):
                if('论文链接' in str(temp[i])):
                    linktmpInfo = getContent(re.findall(findLink, str(temp[i])))
                    if(linktmpInfo !="" ):
                        linkInfo = linktmpInfo
...
def getallcontent(url,txtname):
    ...
    txt = open("./"+path+"/"+path+"-"+txtname+".txt", "a",encoding='utf-8').write("论文链接\n")
    txt = open("./"+path+"/"+path+"-"+txtname+".txt", "a",encoding='utf-8').write(data[3])
...
if __name__=='__main__':
    ...
    getdetailurl("https://news.sjtu.edu.cn/tsfx/index.html",details_url)
    ...
    #数据过多 暂时选取前100页共1200条新闻数据
    for i in range(2,101):
        getdetailurl("https://news.sjtu.edu.cn/tsfx/index_"+str(i)+".html",details_url)
